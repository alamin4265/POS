...
namespace DAL
{
    class CategoryGetWay:MyBase
    {
        ...
        {
            MyCommand = CommandBuilder("select c1.id, c1.name, c1.discription, c2.name as category from category as c1 left join category as c2 on c1.categoryId = c2.id where c1.id > 0");

            if (!string.IsNullOrEmpty(Search))
            {
                MyCommand.CommandText += " and (c1.name like @search or c1.discription like @search)";
